...
// ── Fluorescent clickable feedback ───────────────────────────
...
(function heroCanvas() {
  ...
  var REPEL = 115;
  var R = [230, 57, 70];
  var CODE = ['{  }','( )','//','->','&&','fn','var','if','[ ]','=>','/**','*/','01','10','++','class','new','null'];
  ...
  function mkParticle() {
    ...
    var x = edge === 0 ? Math.random() * W : edge === 1 ? W + 20 : edge === 2 ? Math.random() * W : -20;
    var y = edge === 0 ? -20 : edge === 1 ? Math.random() * H : edge === 2 ? H + 20 : Math.random() * H;
    return {
      x: x, y: y,
  ...
  document.addEventListener('mousemove', function(e) { trackMouse(e.clientX, e.clientY); });
